...
@export var GysroscopeFactor: float = 1.0
@export var RotateParent: bool = false
...
func _input(event):
	...
	if not UseGysroscope and event is InputEventMouseMotion and Input.mouse_mode == Input.MOUSE_MODE_CAPTURED:
		...
		LeftEyePivot.rotate_y(-event.relative.x * Mouse_Sensitivity)
		RightEyePivot.rotate_y(-event.relative.x * Mouse_Sensitivity)

		LeftEyePivot.rotate_object_local(Vector3.RIGHT, -event.relative.y * Mouse_Sensitivity)
		RightEyePivot.rotate_object_local(Vector3.RIGHT, -event.relative.y * Mouse_Sensitivity)

		LeftEyePivot.rotation.x = clamp(LeftEyePivot.rotation.x, deg_to_rad(-80), deg_to_rad(80))
		RightEyePivot.rotation.x = clamp(RightEyePivot.rotation.x, deg_to_rad(-80), deg_to_rad(80))
...
func _process(delta: float) -> void:
	...
	if UseGysroscope:
		...
		if abs(gyroscope.y) > 0.01:
			LeftEyePivot.rotate_y(gyroscope.y * GysroscopeFactor * delta)
			RightEyePivot.rotate_y(gyroscope.y * GysroscopeFactor * delta)

		if abs(gyroscope.x) > 0.01:
			LeftEyePivot.rotate_object_local(Vector3.RIGHT, gyroscope.x * GysroscopeFactor * delta)
			RightEyePivot.rotate_object_local(Vector3.RIGHT, gyroscope.x * GysroscopeFactor * delta)
